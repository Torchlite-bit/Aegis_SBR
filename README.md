...
- **Exorcism (opt-in):** Cast on cooldown against *Undead* and *Demon* targets (creature type is where this starts, not where it ends — one refusal that is not range, sight or cost and Exorcism is dropped for that target, so a mob whose type tag lies cannot hold the rotation), at **priority two, directly behind *Hammer of Wrath*** — ahead of the strike, *Holy Shield*, *Consecration* and the seals, because a rare nuke on its own cooldown should not be waiting for a strike that returns in three seconds. On the Solofarming tab it stays behind the self-heal and *Holy Shield*. Still paused while recovering mana, which is silent and has no opt-out of its own: if it looks ready and never fires, `/sbr trace` names the reason (`exo=MANA MODE`, `exo=wrong type`, ...) along with the target's creature type. **The creature type is only the starting point:** if the client refuses the cast for a reason that is not range, line of sight, cost, facing or cooldown, Exorcism is dropped for that target and the rotation moves on. A tag can be wrong — a quest turns one boss human and leaves the Undead type in place — and without this the whole fight is spent on a spell that cannot go out. Tracked per target, so a new pull tries once and then stays quiet.
...
* **Pet Support:** Pet attack, *Mend Pet* below a health slider, **Kill Command** (it is reactive — the client only allows it after *you* land a critical strike — so it is sent off the global cooldown and lands the moment the window opens), **Bestial Wrath** on its own switch — it grants the pet *Scent of Blood* for 18 seconds, that talent's own +40% pet damage proc, so it is skipped entirely without a live pet — an optional **Baited Shot** fired in the window after the pet crits, and an optional **Smart Pet Taunt** — when the mob peels onto you, the pet's *Growl* is sent to grab it back (off by default; leave it off for melee-weave builds where you want the aggro).
...
> **Verification note:** *Kill Command* and **Lock and Load** are confirmed against the client's own talent data. *Baited Shot* is not — it appears nowhere in the hunter talent tree, which proves nothing on its own (a trainer ability would not be there either) but leaves it the one name here without a source. It is gated by `KnowsSpell`, so a wrong name simply no-ops. If it is not firing, run `/sbr debug` and check the exact name — they drop into one place in `Class_Hunter.lua`. The mana aspect is *Aspect of the Viper* and nothing else: *Aspect of the Beast* was listed here as a second guess and is a different spell entirely — it makes you untrackable and returns no mana — which from level 32 made it the aspect the rotation reached for whenever mana ran low. Auto mode uses `CheckInteractDistance` (~10yd) as its melee proxy; `/sbr trace` shows the effective mode as `mode=auto/melee` or `mode=auto/ranged`.
</details>
...
* **Somebody else's DoT is not yours:** Grouped with a second druid, *Moonfire* and *Insect Swarm* are per caster, so both copies sit on the target at once. The addon now keeps its own when it reads that list, and a copy it can only see as somebody else's no longer answers for yours — it falls back to your own cast record. Without this a druid beside another druid applied its dots on every press and never reached a nuke, recovering only when the other player's copy expired. The same rule now covers two hunters stinging one target, two warlocks, and two shamans.
...
* **Costs come from your spellbook, not a table:** Energy and mana costs are read from the client's own tooltip, so a talent discount is picked up by itself — *Improved Shred* at 2/2 makes Shred 48 rather than 60, and the same holds for every other cat and bear ability with a talent behind it.
...
* **A channel filler channels:** With Drain Life or Drain Soul set as the filler there is no gap to fill — those have no cooldown, unlike *Dark Harvest* — so the next channel starts as soon as the last ends. A DoT that would lapse during one is **topped up first** rather than waited out on the wand, which is the rule Dark Harvest has followed since v1.2.6. As Dark Harvest's *between channels* filler the same holds: it may run a little past Dark Harvest's return rather than standing on the wand for the last seconds of the gap, since a channel tick is worth more than a wand shot. A long channel like *Drain Soul* still stands down — fifteen seconds on top of the cooldown is a different trade. With a channel filler the wand runs **only when the channel is unaffordable** — movement is not an exception, so the rotation keeps its DoTs up and waits rather than wanding. An auto-repeat left running from earlier is stopped on the next press. With *Dark Harvest* as the filler the spell protected is the **gap filler**, since the gap between its channels exists by design and the wand is one of the choices offered for it. Below the configured mana floor the wand always runs: that valve exists to stop the rotation casting itself dry.
* **You set how much DoT time a channel needs:** two sliders under *Filler and pet*. **Top up DoTs before Dark Harvest** (default 10s) and **before other channels** (default 5s) decide how much time a DoT must have left before the channel is allowed to start; anything shorter is re-applied first. Split in two because Dark Harvest accelerates the DoTs already on the target — one dropping out mid-channel loses that boost for the rest of it — while the other channels do nothing for them and only need the DoTs to keep running alongside. **0 starts the channel whatever the DoTs are doing.** The defaults are the values that used to be hardcoded.
...
* **Talent-Aware Nightfall & Rapid Deterioration:** Aegis reads your **talent tree** to detect *Nightfall* and **auto-fires the free instant *Shadow Bolt*** the moment *Shadow Trance* procs — checked ahead of every other priority (including Drain Life) so a longer action started first can never burn through the whole proc window unused. The proc is spent **once per proc**, and a **fresh proc off the bolt that spent the last one** is picked up rather than sat on: a lingering icon counts down, a new proc restarts the timer, and only the second releases the latch. That latch is maintained ahead of every guard, so a proc landing during a channel is still there when the channel ends. On a Nightfall build Shadow Bolt is **never hard cast** unless you set it as the filler yourself — three seconds of casting is the most expensive press an affliction rotation can make. *Rapid Deterioration* (2 ranks, 3% shorter *Corruption* / *Curse of Agony* / *Siphon Life* / *Dark Harvest* duration per rank) is likewise read from your talent rank and scales every duration estimate the rotation makes.
...
* **One spell in flight:** Nampower's queue holds exactly one spell, so a second send does not join a line — it replaces what is there. The rotation presses four times a second and used to send on every one of them, which threw away the spell it had just asked for. It now sends once and waits for the client to answer — a cast starting, a channel starting, or a refusal — before deciding again. Once the client has taken the spell, re-deciding resumes and the next spell queues behind it, which is what the queue is for.

# Aegis: Single Button Rotation (v1.2.8)
...
- **Consecration can wait for a crowd:** An **Only with enemies nearby** slider (0–5, off by default) holds it until that many enemies are actually standing in it, with the radius read from the spell's own tooltip. 1.12 has no API for this, so the count comes from the **nameplates the client is drawing** — SuperWoW turns each one into a unit that can be asked its distance. With nameplates off there is nothing to enumerate, and that is treated as *unknown* rather than as zero, so Consecration still goes out: a capability that cannot answer never switches an ability off silently. It counts what you can see and is not a radar.

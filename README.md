...
1. Place the folder directly into your World of Warcraft directory under: `Interface\AddOns\`
   *(Ensure the folder name matches the `.toc` file exactly: `Interface\AddOns\AutoRota\`)*
1. Log into the game. Make sure “Load OutofDate AddOns” is checked if prompted.
...
|`/ar heal <on/off>`                         |*(Paladin Only)* Toggles heal mode (group healer that DPSes between heals; works at range).                                                              |`/ar heal on`            |
|`/ar healat <1-100>`                        |*(Paladin Only)* Heals group members below this % health.                                                                                                |`/ar healat 85`          |
|`/ar hsat <1-100>`                          |*(Paladin Only)* Health % under which *Holy Shock* is used as an instant emergency heal.                                                                 |`/ar hsat 50`            |
|`/ar healpower <n>`                         |*(Paladin Only)* Manual +healing override for downranking (0 = auto-read from gear).                                                                     |`/ar healpower 0`        |

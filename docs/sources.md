...
| IWinEnhanced — https://github.com/… (unpacked locally by the user) | ✅ Local source | **A comparable 1-button rotation addon, worth reading rather than guessing.** Where its answers came from: `CleveRoids.CountEnemiesMatching` (nameplate GUIDs) for the enemy count, `UnitXP("behind", …)` for facing, `GetItemInfo` subtype for weapon requirements, and a per-press condition cache. It hard-depends on SuperCleveRoidMacros + UnitXP + Nampower and does **no healing at all** (`priest/action.lua` has none), so it is a source for damage-rotation mechanics only | 2026-09-01 |
